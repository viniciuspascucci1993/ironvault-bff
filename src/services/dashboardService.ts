...
export const dashboardService = {
  getSummary: async (token: string) => {
    const [paymentsResponse, usersResponse] = await Promise.allSettled([
    ...
    const users =
      usersResponse.status === "fulfilled" ? usersResponse.value.data : [];

    const totalTransactions = payments.totalElements || 0;
    const totalUsers = users.length || 0;

    const approved =
      payments.content?.filter((p: any) => p.status === "APPROVED").length || 0;
    const failed =
      payments.content?.filter((p: any) => p.status === "FAILED").length || 0;
    const processing =
      payments.content?.filter((p: any) => p.status === "PROCESSING").length ||
      0;
